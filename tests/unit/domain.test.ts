...
describe('writing domain', () => {
  ...
  it('isolates null and malformed model issues without throwing', () => {
    ...
    expect(validateResponse({ schemaVersion: '1', requestId: 'r', documentRevision: 1, units: [{ unitId: 'a', unitRevision: 1, issues: [null] }] }, expected).rejected).toEqual(['a']);
  });
